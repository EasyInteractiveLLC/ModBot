...
class userinfo(commands.Cog):
  ...
  @commands.hybrid_command(description="User Information")
  ...
  async def userinfo(self, ctx, member: discord.Member = None):
    ...
    if member is None:
      ...
      embed = discord.Embed(title=f"{member}", color=discord.Color.blue(), timestamp=ctx.message.created_at)
      ...
      embed.add_field(name="Created At:", value=f"<t:{round(member.created_at.timestamp())}:F>")
      embed.add_field(name="Joined At:", value=f"<t:{round(member.joined_at.timestamp())}:F>")
      ...
      embed2 = discord.Embed(title=f"{member}", color=discord.Color.blue(), timestamp=ctx.message.created_at)
      ...
      embed2.add_field(name="Created At:", value=f"<t:{round(member.created_at.timestamp())}:F>")
      embed2.add_field(name="Joined At:", value=f"<t:{round(member.joined_at.timestamp())}:F>")
