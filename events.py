...
class events(commands.Cog):
  ...
  async def on_ready(self, ctx: commands.Context = None):
    await self.bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"Channel | m.help"))
    # cur = conn.cursor()
  ...
  async def on_message(self, ctx):
    if self.bot.user.mentioned_in(ctx) and len(ctx.content.split(' ')) == 1 and ctx.content[-1] == ">" and ctx.content[0] == '<':
      await ctx.channel.send("My prefix is `m.`\nTry `m.help` for help with commands")
